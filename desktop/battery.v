...
fn battery_settings_elements(percent int, x int, inner int) []ui2.Element {
	...
	mut children := [
		ui2.label('settings.battery.title', 'Battery', ui2.rect(f64(x), 16, f64(inner), 28), ui2.TextStyle{ color: body_heading, size: 20, bold: true }),
		...
		ui2.label('settings.battery.percent', if available {
			battery_percentage_text(percent)
		} else {
			'Unavailable'
		}, ui2.rect(f64(x), 118, f64(inner), 40), ui2.TextStyle{
			color: if available { body_heading } else { body_muted }
			size: 28
			bold: true
		}),
		ui2.view('settings.battery.track', ui2.rect(f64(x), 172, f64(inner), 12), ui2.BoxStyle{ bg: body_rule, radius: 4 }, []),
		settings_label(battery_status_text(percent), x, 200, inner, if available {
			body_muted
		} else {
			files_error
		}),
