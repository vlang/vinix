...
static int a_sart_allow(struct ans *a, uint64_t pa, unsigned bytes)
{
    ...
    for (unsigned i = 0; i < 16; ++i) {
        ...
        a->sart_pa[i] = pa; a->sart_bytes[i] = bytes;
...
{
    if (q == 1) return c[0] <= 2; /* NVM Flush, Write, Read */
    if (q) return 0;
    switch (c[0]) {
    case 0: case 4: case 1: case 5: return a_le16(c + 40) == 1; /* Create I/O SQ/CQ 1 */
...
static int a_authorize(struct ans *a, unsigned q, const uint8_t c[64])
{
    if (!a_command_allowed(q, c)) return 0;
    if (!q) {
        if (c[0] == 0 || c[0] == 4) return a->stopping && a->ioq_active;
        return !a->stopping;
    }
    if (c[0] == 2) return !a->stopping;
    unsigned i = 0;
    for (; i < a->nns && a->ns[i].id != a_le32(c + 4); ++i) {}
    if (i == a->nns) return 0;
    if (c[0] == 0) {
        for (unsigned j = 8; j < 64; ++j) if (c[j]) return 0;
        return c[1] == 0;
    }
    if (!a->write_enabled || a->write_fault || a->stopping || i != a->write_ns ||
        c[1] || a_le64(c + 16) || a_le64(c + 24) != a->physical + BOUNCE ||
        a_le16(c + 50) != 0x4000u) return 0;
    for (unsigned j = 8; j < 16; ++j) if (c[j]) return 0;
    for (unsigned j = 52; j < 64; ++j) if (c[j]) return 0;
    uint64_t lba = a_le64(c + 40), n = (uint64_t)a_le16(c + 48) + 1;
    if (lba < a->write_start || lba - a->write_start >= a->write_blocks ||
        n > a->write_blocks - (lba - a->write_start) || n > a->max_transfer / a->ns[i].sector) return 0;
    unsigned bytes = (unsigned)n * a->ns[i].sector;
    uint64_t prp2 = bytes <= PAGE ? 0 : a->physical + (bytes <= 2 * PAGE ? BOUNCE + PAGE : PRPL);
    if (a_le64(c + 32) != prp2) return 0;
    if (bytes > 2 * PAGE)
        for (unsigned page = 1; page < (bytes + PAGE - 1) / PAGE; ++page)
            if (a_le64(a->dma + PRPL + (page - 1) * 8) != a->physical + BOUNCE + page * PAGE) return 0;
    return 1;
}
...
{
    if (a->dead) return -a->error;
    if (a->stopped) return -ANS_STOPPED;
    if (!a_authorize(a, qid, c)) return -ANS_READ_ONLY;
...
{
    if (a->dead) return -a->error;
    if (a->stopping || a->stopped) return -ANS_STOPPED;
...
int vinix_ans_boot_flags(const char *s, size_t n)
{ struct ans_policy p; return a_parse_policy(s, n, &p) ? -ANS_CONFIG : (int)p.flags; }
...
static int a_root_disk(void *cookie, void *buffer, uint64_t offset, size_t count)
{
    struct ans *a = cookie;
    if (!a->live || !a->root_selected) return -ANS_STOPPED;
    struct ans_namespace *ns = &a->ns[a->root_ns];
    struct ans_partition *p = &ns->parts[a->root_part];
    uint64_t bytes = p->blocks * ns->sector;
    if (offset > bytes || count > bytes - offset) return -ANS_RANGE;
    return a_read_bytes(a, a->root_ns, buffer, p->start * ns->sector + offset, count);
}
static int a_open_root(struct ans *a, void *context, size_t size)
{
    if (!a->live || !a->root_selected) return -ANS_STOPPED;
    const struct ans_namespace *ns = &a->ns[a->root_ns];
...
int vinix_ans_apply_policy(const char *s, size_t n)
{ struct ans_policy p; int rc = a_parse_policy(s, n, &p); return rc ? rc : a_apply_policy(&controller, &p); }
int vinix_ans_partition_writable(unsigned i, unsigned p)
{ return controller.live && controller.write_enabled && !controller.write_fault && !controller.stopping &&
    i == controller.write_ns && p == controller.write_part; }
int vinix_ans_partition_uuid(unsigned i, unsigned p, char *out, size_t n)
{ if (!out || n < 37 || i >= controller.nns || p >= controller.ns[i].nparts) return -ANS_RANGE;
  a_guid_format(controller.ns[i].parts[p].guid, out); return 0; }
int vinix_ans_write(unsigned i, unsigned p, const void *buf, uint64_t off, size_t n)
{ return a_write_partition(&controller, i, p, buf, off, n); }
int vinix_ans_flush(void) { return a_flush_all(&controller); }
int vinix_ans_shutdown(void) { return a_shutdown(&controller); }
int vinix_ans_root_ns(void) { return controller.root_selected ? (int)controller.root_ns : -1; }
int vinix_ans_root_part(void) { return controller.root_selected ? (int)controller.root_part : -1; }
...
{ return controller.live ? vinix_ext2_stat(a_root_context, ino, fields) : -ANS_STOPPED; }
int64_t vinix_ans_root_read(uint32_t ino, void *buffer, uint64_t offset, size_t count)
