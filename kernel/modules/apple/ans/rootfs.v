...
fn (mut this AnsRootResource) read(_handle voidptr, buf voidptr, loc u64, count u64) ?i64 {
	if !stat.isreg(this.stat.mode) { errno.set(errno.eisdir); return none }
	if loc >= u64(this.stat.size) { return 0 }
	mut bytes := count
	...
	for done < bytes {
		...
		ans_lock.release()
		if result != i64(chunk) { errno.set(errno.eio); return none }
		done += chunk
	}
	return i64(done)
}
...
fn (mut this AnsRootResource) mmap(_handle voidptr, page u64, flags int) voidptr {
	...
	if n > 4096 { n = 4096 }
	ans_lock.acquire()
	result := C.vinix_ans_root_read(u32(this.stat.ino),
		voidptr(u64(physical) + memory.get_hhdm_offset()), page * 4096, n)
	ans_lock.release()
	if result != i64(n) {
		memory.pmm_free(physical, 1)
		return unsafe { nil }
	}
	// Keep ownership of the immutable source page. mmap's private-fault path
	// copies it; returning a disposable allocation would leak on every fault.
	this.pages[page] = physical
	// PMM zeroed the whole page; bytes after EOF remain zero.
	return physical
}
...
fn (mut this AnsRootResource) grow(_h voidptr, _size u64) ? { errno.set(errno.erofs); return none }
fn (mut this AnsRootResource) unlink(_h voidptr) ? { errno.set(errno.erofs); return none }
fn (mut this AnsRootResource) link(_h voidptr) ? { errno.set(errno.erofs); return none }
...
fn (mut this AnsRootFS) instantiate() &fs.FileSystem { return &AnsRootFS{dev_id: this.dev_id} }
...
fn build_root() ?&fs.VFSNode {
	...
	ans_lock.release()
	if result != 0 { println('ans-root: unsupported, unclean, or unreadable ext2'); return none }
	...
	for head := 0; head < todo.len; head++ {
		...
		if directory_bytes > 0x4000000 || entry.depth > 32 { return none }
		mut position := u64(0)
		mut dots := 0
		for {
			...
			if name == '.' || name == '..' {
				expected := if name == '.' || node.parent == unsafe { nil } { node.resource.stat.ino } else { node.parent.resource.stat.ino }
				bit := if name == '.' { 1 } else { 2 }
				unsafe { name.free() }
				if u64(inode) != expected || dots & bit != 0 { return none }
				dots |= bit
				continue
			}
			if name in node.children || total_nodes >= 32768 { unsafe { name.free() }; return none }
			...
			if stat.isdir(child.resource.stat.mode) {
				if inode in directories { return none } // directory hard link/cycle
				directories[inode] = true
				todo << RootDirectory{node: child, depth: entry.depth + 1}
			}
		}
		if dots != 3 || total_nodes > 32766 { return none }
		...
		unsafe { node.children['.'] = dot; node.children['..'] = dotdot }
	}
	return root
}
...
pub fn select_root() bool {
	if ans_policy_invalid { return false }
	if ans_boot_flags & 4 == 0 { return true }
	if ans_ready {
