...
fn (mut z Zone) zone_meta_remqueue(mut p ZoneChunk) {
	...
	unsafe { assert p.owner == u64(&z) }
	if p.prev == unsafe { nil } {
		unsafe { assert u64(z.queue_head(p.queue)) == u64(&p) }
...
pub fn (mut z Zone) zone_element_resolve(addr u64) &ZoneChunk {
	if z.resolve != unsafe { nil } {
		p := z.resolve(addr)
		if p != unsafe { nil } && p.owner == unsafe { u64(&z) }
			&& z.slot(p, addr) != no_element {
			return p
		}
		return unsafe { nil }
	}
	// Portable/test fallback. A Vinix resolver avoids this list traversal.
	mut p := z.zone_element_resolve_from(z.full, addr)
	if p != unsafe { nil } {
		return p
	}
	p = z.zone_element_resolve_from(z.partial, addr)
	if p != unsafe { nil } {
		return p
	}
	p = z.zone_element_resolve_from(z.empty, addr)
	if p != unsafe { nil } {
		return p
	}
	return unsafe { nil }
...
fn (z &Zone) zone_element_resolve_from(head &ZoneChunk, addr u64) &ZoneChunk {
	unsafe {
		mut p := head
		for p != nil {
