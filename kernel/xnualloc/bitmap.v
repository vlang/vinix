...
// Spell the sentinel as a positive literal so V emits a static C constant.
// u64(-1) is initialized by _vinit, but Vinix brings its heap up before _vinit.
pub const no_element = u64(0xffffffffffffffff)
...
pub fn zba_scan_bitmap_ref(bits &u64, words u32, eidx u64) u64 {
	...
	unsafe {
		...
		for j := u32(0); j < words; j++ {
			...
			if word != 0 {
				bits[i] &= word - 1
				return i * 64 + trailing_zeros(word)
			}
			i++
		}
	}
...
pub fn zba_scan_bitmap_inline(meta &PageMetadata, chunk_len u32, eidx u64) u64 {
	...
	unsafe {
		if eidx % 32 != 0 {
			word := meta[i].bitmap & (u32(-1) << (eidx % 32))
			if word != 0 {
				bit := trailing_zeros(u64(word))
		...
		for j := u32(0); j < chunk_len; j++ {
			...
			word := meta[i].bitmap
			if word != 0 {
				meta[i].bitmap &= word - 1
				return i * 32 + trailing_zeros(u64(word))
			}
			i++
		}
	}
