...
def tree_key(paths: list[Path], metadata_only: bool) -> str:
    ...
    for index, path in enumerate(paths):
        CONTENT_KEY.hash_path(
            digest, path, f"root-{index}".encode(), metadata_only
        )
    return digest.hexdigest()
...
def compute_key(root: Path, v_compiler: Path, env: dict[str, str]) -> str:
    ...
    sysroot = resolved_env_path(
        env, "VINIX_AARCH64_SYSROOT", userland_build / "staging"
    )
    ...
    network_staging = resolved_env_path(
        env,
        "VINIX_NETWORK_TOOLS_STAGING",
        root / "build-aarch64-network-tools/staging",
    )
    ...
    libreoffice_staging = resolved_env_path(
        env,
        "VINIX_LIBREOFFICE_STAGING",
        root / "build-aarch64-libreoffice/staging",
    )
    ...
    blender_staging = resolved_env_path(
        env,
        "VINIX_BLENDER_NATIVE_STAGING",
        root / "build-aarch64-blender-native/staging",
    )
    x86_staging = resolved_env_path(
        env,
        "VINIX_X86_TRANSLATION_STAGING",
        root / "build-aarch64-x86-translation/staging",
    )
    ...
    llvm_bin = Path(
        os.path.expanduser(env.get("LLVM_BIN", "/opt/homebrew/opt/llvm/bin"))
    )
