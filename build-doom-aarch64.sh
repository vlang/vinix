#!/bin/bash
# Build upstream Chocolate Doom and stage it with SDL2 and SDL2_mixer for the
# AArch64 desktop. The WAD is local input and is never checked into the
# repository.
set -euo pipefail
...
# Sound effects and the OPL music emulation both play through SDL2_mixer.
# Alpine's build loads its codecs on demand and links only SDL2 and musl.
SDL_MIXER_VERSION=2.8.0-r0
...
for package in sdl2-$SDL_VERSION sdl2-dev-$SDL_VERSION \
    sdl2_mixer-$SDL_MIXER_VERSION sdl2_mixer-dev-$SDL_MIXER_VERSION; do
    archive="$DOWNLOADS/$package.apk"
    if [ ! -s "$archive" ]; then
        curl -fL --retry 3 -o "$archive" \
            "https://dl-cdn.alpinelinux.org/alpine/v3.21/community/aarch64/$package.apk"
    fi
...
cmake -S "$SOURCE" -B "$BUILD_DIR/cmake" -G Ninja \
    ...
    -DENABLE_SDL2_NET=OFF -DENABLE_SDL2_MIXER=ON \
    ...
    -DSDL2_MIXER_INCLUDE_DIR="$SDL_ROOT/usr/include/SDL2" \
    -DSDL2_MIXER_LIBRARY="$SDL_ROOT/usr/lib/libSDL2_mixer.so" \
...
install -m755 "$SDL_ROOT/usr/lib/libSDL2_mixer-2.0.so.0.800.0" \
    "$STAGING/usr/lib/libSDL2_mixer-2.0.so.0"
if [ -f "$WAD" ]; then
