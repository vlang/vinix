...
def load_library(shift: int, destination: Path) -> C.CDLL:
    ...
    if sys.platform == 'darwin':
        # System Python is hardened and cannot load Apple's dynamic UBSan
        # runtime. Trap instrumentation keeps the shared library self-contained
        # and still makes undefined behavior fail the test process.
        sanitizer_flags = ['-fsanitize=undefined', '-fsanitize-trap=undefined']
    command = [os.environ.get('CC', 'cc'), '-std=c11', '-D_POSIX_C_SOURCE=200809L',
               f'-DPAGE_MAX_SHIFT={shift}', '-O1', '-g', '-Wall', '-Wextra', '-Werror',
               *sanitizer_flags, '-shared', '-fPIC',
