...
cc=${CC:-clang}
work=$(mktemp -d "${TMPDIR:-/tmp}/vinix-realtime.XXXXXX")
...
echo "==> Building static AArch64 real-time test init..."
"$cc" --target=aarch64-linux-musl --sysroot="$sysroot" \
	-static -O2 -fno-stack-protector -Wall -Wextra -Werror \
	"$repo/tests/realtime/test.c" -L"$sysroot/lib" -fuse-ld=lld \
	-o "$work/init"
...
mkdir -p "$work/rootfs/root" "$work/rootfs/sbin" "$work/rootfs/sys" "$work/rootfs/proc"
COPYFILE_DISABLE=1 tar --format=ustar -cf "$work/initramfs.tar" \
	-C "$work/rootfs" .

python3 "$repo/tests/realtime/run_vm.py" \
	--init "$work/init" \
