...
The read-only battery driver is enabled by default on ARM64 and safely declines
device trees without a compatible Apple SMC. The exact kernel command-line
option `vinix.apple_battery=0` disables it; `vinix.apple_battery=1` re-enables
it, and the last battery option wins. Keep a known-good boot entry. This is
independent of the GPU/DCP options. Rebuild using the existing working ARM64/M1
build procedure; the kernel makefile discovers the C source.
...
are unsupported. A missing device or initially absent BUIC and BRSC leaves the
node absent.
...
Only SMC GET_SRAM_ADDR (0x17) and READ_KEY (0x10) are sent. Current firmware's
BUIC/u8 capacity key is preferred, with BRSC/ui16 as a read-only fallback for
older firmware. Status, message ID, eight-bit response size and the 0..100 range
...
Boot negotiation has a two-second counter deadline; commands have 500 ms
...
The 25 host test groups exercise the real C core against a simulated peer:
...
These tests and sanitizer checks passed during development, as did a complete
AArch64 kernel build. Real M1 hardware has NOT been validated here. For hardware
testing retain a known-good kernel, inspect diagnostics, exercise one-byte and
concurrent reads, and verify that a freshly opened device tracks charge changes.
