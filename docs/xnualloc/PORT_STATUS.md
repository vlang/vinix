...
Updated 2026-09-11. The allocator-only commits from draft PRs #178 and #179 are
integrated into `master`, followed by compile, production-test and boot
fixes. **This remains an adapted, partial translation, not a completed
transplant of every XNU allocator path or configuration.**

## Provenance

Vinix inputs: PR #178 head `1dc68880818947654a69de85193d8800be0e4749` and
the three allocator continuation commits ending at PR #179 head
`183c323f`. PR #179 also contains the unrelated ancestor
`7c085707f535e498ff2ce9a6447d8a5b59844388` (an x86 TSS/I/O change); that commit
...
| Vinix small heap entry points and early/SMP registration | `memory/xnu_zone_heap*.v`, routing in `physical.v`/`slab.v`, both SMP initializers | Source-connected behind `-d xnu_zone`; compiled on x86_64/AArch64 and boot-tested on x86_64. |
...
## Validation, boot and performance
...
- Actual V host suite with V 0.5.2 and GCC passed in debug and `-prod`: 18
  functions across `port_test.v`, `zone_test.v` and `smoke_test.v`.
- Flag-off, `-d xnu_bitmap` and `-d xnu_zone` kernels compiled and linked in
  debug mode for both x86_64 and AArch64. Production x86_64 flag-off and zone
  benchmark kernels also compiled and linked.
- A four-vCPU x86_64 QEMU boot of the XNU backend reached
  `heap: self-test passed`, completed the cache-enabled benchmark and continued
  into the Alpine userspace. The self-test exposed and prompted a fix for an
  early-boot sentinel that had depended on `_vinit`.
- `git diff --check` passed for the integration fixes.

The protocol model is sequential under the one-lock assumption; it is not a
hardware or weak-memory-order concurrency test. The boot self-test uses public
malloc/free and exercises the selected backend, but runs before SMP and does
not by itself test cache registration.

The production in-kernel comparison used three alternating one-vCPU QEMU/TCG
boots per backend and 15 warmed samples per workload. The XNU zone was 50.19%
slower for hot 64-byte allocate/free pairs and 5.83% faster for mixed-size
batches. It permanently retained 56 KiB of cache metadata and held 176 KiB of
payload before trim versus 32 KiB for the Vinix bitmap backend; both reclaimed
all measured payload after trim. See [BENCHMARK.md](BENCHMARK.md) for the exact
method, medians, ranges, memory results and limitations.

Still required before default enablement: interrupt-heavy concurrent cross-CPU
allocation/free/trim, OOM and fault injection, native-hardware throughput and
tail latency, and lock-contention/pressure measurements. See
`tests/xnualloc/README.md` for exact commands.
